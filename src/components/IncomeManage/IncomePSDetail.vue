    ...
    <div class="income-dht-detail" v-for="item in lists">
      ...
      <div class="income-dht-detail_block">
        <div class="income-dht-detail_item">
          <div class="item">
            <div class="income-dht-detail_item_name">{{item.userName}}</div>
            <p class="font-color-gray">{{item.CUSTOMER_PHONENUM}}</p>
          </div>
          <div class="item text-right">
            <div class="income-dht-detail_item_name">{{item.productName
}}</div>
  ...
  export default {
    ...
    methods: {
      getDetail (time, page, rows) {
        this._post(this, {SERVERID: '60000024', 'DATA_DATESTR': time, page: page, rows: rows}, (response) => {
